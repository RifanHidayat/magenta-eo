	...
	if ($idd=="QE"){
     ...
     $this->form_validation->set_rules('Quatations_number','Quotation','required');
      ...
      $this->data['img']=$img['image'];
			...
			$this->aksi_update_faktur_event($id);
...
$arr['success'] = true;
  ...
  if ($idd=="QE"){
     ...
     $this->db->select('faktur_number,ser_faktur,date_faktur,syarat_pembayaran,REF,faktur.ppn,faktur.pph23,diskon,total_faktur,total_sub,name,address,comissionable_cost,nonfee,tittle_event,total_summary,asf,npwp,diskon_harga,po_number,gr_number,totalBast,asfp');
     $this->db->from('faktur');
     $this->db->join('bast','bast.id_bast = faktur.id_bast');
  ...
  if(in_array('updateFaktur', $this->permission)){
                ...
                if(in_array('createDelivery', $this->permission) AND $row->status=="Close"){
                  ...
                  $data1=$this->db->get()->row_array();
                  if (($data1!='') AND ($data1['id_faktur']==$row->id_faktur)){                           $delivery='<font color="#FFFFFF" size="2px"><a title="Edit Delivery" class="btn btn-sm bg-gradient-secondary" onclick="cekDelivery('.$row->id_faktur.')" ><i class="fa fa-check"></i>   Delivery</a></font>';
              ...
              if (($row->statusQuotations=="Final")  AND ($row->statusBast=="Close")){
                ...
                $sub_array[] = $edit.' '.$delete.' '.$print.' '.$view.' '.$delivery;
                $data[] = $sub_array; 
