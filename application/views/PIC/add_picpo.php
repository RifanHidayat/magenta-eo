...
----------------------------------------------------------------------------------------------------------------------- -->
                    ...
                    <form action="<?php echo base_url('PicPO/aksi_add_pic') ?>" method="post" id="SimpanDataPO" name="formidevent" class="form-horizontal" enctype="multipart/form-data">
                      <div class="box-body">
                      ...
                      <button type="submit" class="btn btn-primary btnSavePO" type="button">
                        <span class="spinner-border spinner-border-sm loadingIndikdatorPO" role="status" aria-hidden="true"></span>
                        Save
                      </button>
...
----------------------------------------------------------------------------------------------------------------------- -->
                <div class="tab-pane fade" id="custom-tabs-one-messages" role="tabpanel" aria-labelledby="rcustom-tabs-one-messages-tab">
                  ...
                  <form action="<?php echo base_url('PicPO/aksi_add_pic_event') ?>" method="post" id="SimpanDataEvent" name="formidevent" class="form-horizontal" enctype="multipart/form-data">
                    <div class="box-body">
                    ...
                    <button type="submit" class="btn btn-primary btnSaveEvent" type="button">
...
<script type="text/javascript">
  function showIndikatorPO() {
    $('.btnSavePO').attr('disabled', 'disabled');
    $('.loadingIndikdatorPO').show();
  }


  function hiddenIndikatorPO() {
    $('.btnSavePO').removeAttr('disabled');
    $('.loadingIndikdatorPO').hide();
  }

  function showIndikatorEvent() {
    $('.btnSaveEvent').attr('disabled', 'disabled');
    $('.loadingIndikdatorEvent').show();
  }

  function hiddenIndikatorEvent() {
    $('.btnSaveEvent').removeAttr('disabled');
    $('.loadingIndikdatorEvent').hide();
  }

  $('#SimpanDataEvent').submit(function(e) {
    e.preventDefault();
    showIndikatorEvent();
    $('#SimpanDataEvent').submit();
  });
  $('#SimpanDataPO').submit(function(e) {
    e.preventDefault();
    showIndikatorPO();
    $('#SimpanDataPO').submit();
  });
