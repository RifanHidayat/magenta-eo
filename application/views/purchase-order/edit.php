    ...
    <h1 class="card-title">Summary</h1>
        ...
        <table class="table" v-if="selectedProducts.length > 0">
            ...
            <tbody>
                ...
                <td><input type="text" class="form-control" v-model="product.price" @input="calculateAmountProduct(product)"></td>
    ...
    $('#datatableProducts tbody').on('click', '.btn-choose', function() {
        ...
        axios.get(`${API_URL}/api/purchase-order/<?php echo $id ?>/product`).then((response)=>{
         ...
         response.data.data.map((item,index)=>{
             item['amount']=(item['price']*item['quantity']) -item['discount'];
...
// let API_URL="http://localhost:3000";
    ...
    let   app=new  Vue({
        ...
        data:{
            code:"<?php echo $code ?>",
            suppliersId: '<?php echo $supplier_id ?>',
            accountsId:'<?php echo $account_id ?>',
        ...
        methods: {
            ...
            //  console.log(this.selectedProducts)
                ...
                axios.patch(`${API_URL}/api/purchase-order`,{
                    code:vm.code,
                    date:vm.date,
        ...
        computed: {
            netTotal: function() {
                const netTotal = this.selectedProducts.map(product => {
                        const amount = (Number(product.quantity) * product.price) - Number(product.discount);
