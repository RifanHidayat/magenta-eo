            ...
            <div class="card card-primary card-tabs">
              ...
              <div class="card-header p-0 pt-1">
                <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                  <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill" href="#custom-tabs-one-home" role="tab" aria-controls="custom-tabs-one-home" aria-selected="true"><b><h4>Quotation</h4></b></a>
                  </li>
...
----------------------------------------------------------------------------------------------------------------------- -->
             ...
             <form  action="<?php echo base_url('Quotation/aksi_add_quotation') ?>" method="post" id="SimpanData1" name="formidevent" class="form-horizontal" enctype="multipart/form-data">
  ...
  ?>
            ...
            <div class="col-md-6 col-xs-12 pull pull-left" >
                ...
                <div class="form-group" id="qnumber">
                  ...
                  <div class="col-sm-12">
                  <input type="date" required="" class="form-control" id="date_quotation_event" name="date_quotation_event"  autocomplete="off" value="<?php echo set_value('Date_quotation')?>" >
                </div>
                  ...
                  <div class="col-sm-12">
                  <input type="date" class="form-control" required="" id="date_expired_event" name="date_expired_event" autocomplete="off" value="<?php echo set_value('date_expired_event')?>" >
                </div>
               ...
               <?= form_error('customer_event','<small class="text-danger pl-3">','</small>')?>
                ...
                <div class="form-group" id="qnumber">
                   ...
                   <div class="col-sm-12">
                     <input type="email" readonly class="form-control" id="customerEvent" name="customerEvent" autocomplete="off" value="<?php echo set_value('customerEvent')?>">
...
----------------------------------------------------------------------------------------------------------------------- -->
                  <div class="tab-pane fade" id="custom-tabs-one-messages" role="tabpanel" aria-labelledby="rcustom-tabs-one-messages-tab">
                    <form action="<?php echo base_url('Quotation/aksi_add_quotation_other') ?>" method="post" id="SimpanData" name="formid" class="form-horizontal" enctype="multipart/form-data">
                 ...
                 <!--   other -->
                  ...
                  <label for="pph"style="text-align:left;" class="col-sm-6 control-label" >Image Eo</label>
                   <div class="col-sm-12">
                 <input id="otherr" name="other" type="file" required="">
  ...
  ?>
            ...
            <div class="col-md-6 col-xs-12 pull pull-left" >
                ...
                <div class="form-group" id="qnumber">
                  ...
                  <div class="col-sm-12">
                  <input type="date" class="form-control" required="" id="date_quotation" name="date_quotation" autocomplete="off" value="<?php echo set_value('Date_event')?>" >
                </div>
                     <?= form_error('Date_event','<small class="text-danger pl-3">','</small>')?>
                   </div>
                    <div class="form-group" id="qnumber">
                  ...
                  <div class="col-sm-12">
                  <input type="date" class="form-control" required="" id="date_expired_other" name="date_expired_other" autocomplete="off" value="<?php echo set_value('date_expired_other')?>" >
                </div>
                     <?= form_error('Date_event','<small class="text-danger pl-3">','</small>')?>
                   </div>
    ...
    // -------------------------Membuat Table baru comminable cost-----------------------------------
      ...
      function hitunggrandtotal<?php echo (str_replace(' ', '', $k->name))?>(){
        ...
        for (x = 1; x <= Nomor; x++) {
          ...
          var q=$("#Quantity<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
          var f=$("#Frequency<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
          var r=$("#Rate<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
          var ra = r.replace(/[^\w\s]/gi, '');
          //var s=$('#subtotal'+x).val();
          
          var s=Number(q)*Number(f)*Number(ra);
          var ss = s.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.");
           $("#subtotal<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val(ss);
            $("#subtotalhidden<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val(s);
           var subtotal=$("#subtotalhidden<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
...
//----------------------------membuat table non ------------------------------------------
    ...
    function hitunggrandtotalnonfee<?php echo (str_replace(' ', '', $k->name))?>(){
        ...
        for (x = 1; x <= Nomor; x++) {
          ...
          var q=$("#QuantityNON<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
          var f=$("#FrequencyNON<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
          var r=$("#RateNON<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
          var ra = r.replace(/[^\w\s]/gi, '');
          //var s=$('#subtotal'+x).val();
          
          var s=Number(q)*Number(f)*Number(ra);
          var ss = s.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.");
           $("#subtotalNON<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val(ss);
            $("#subtotalhiddenNON<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val(s);
           var subtotal=$("#subtotalhiddenNON<?php echo (str_replace(' ', '', $k->name)) ?>"+x).val();
           hitung=Number(hitung)+Number(subtotal);
                  ...
                  var Baris = '<tr id=trdescription'+Nomor+'>';
                          Baris +=  '<td></td>';
                           ...
                           Baris += '<td style="width:30%">';
                              Baris += '<input  type="text" name="description[]" id="description'+Nomor+'"  class="form-control deposit"  required="" >';
                          Baris += '</td>';
                ...
                reader.readAsDataURL(input.files[0]);
                 $("#imageother").show();
                   ...
                   // hitungDescription();
                      ...
                      var jml=$("#tableLoopDescription tbody tr").length;
                      for (x=0;x<=jml;x++){
                           $('#trdescription'+x).remove();
      ...
      $.ajax({
          ...
          success:function(hasil){
              ...
              console.log(hasil);
               ...
               $('[name="emailEventOther"]').val(hasil[0].pic_name);
