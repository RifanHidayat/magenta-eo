...
<div class="content-wrapper">
  ...
  <div class="content" id="app">
    ...
    <span class="text-black ml-3 text-xl  " >Detail Transaksi</span><br>
...
$( document ).ready(function() {
    ...
    $('#datatableProducts tbody').on('click', '.btn-choose', function() {
         ...
         //get payment History
         axios.get(`${API_URL}/api/purchase-transaction/<?php echo $purchase_order_id ?>`).then((response)=>{
          const transactions=app.$data.transactions;

          const purchaseTransaction=response.data.data.filter((item=>item.id==<?php echo $id?>))
        
          console.log(response.data.data)
         purchaseTransaction.map((item)=>{
        ...
        //get products
        axios.get(`${API_URL}/api/purchase-order/<?php echo $purchase_order_id ?>/product`).then((response)=>{
          const selectedProducts=app.$data.selectedProducts;
...
// let API_URL="http://localhost:3000";
    ...
    let   app=new  Vue({
        ...
        data:{
            ...
            pruchaesOrderId:'<?php echo $purchase_order_id ?>',
        ...
        methods: {
            ...
            calculateAmountProduct:function(product){
                product.amount=(product.quantity * product.purchase)-product.discount;
